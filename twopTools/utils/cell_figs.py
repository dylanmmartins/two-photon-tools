""" Figures to help reviewing cells.
twopTools/utils/cell_figs.py
...
def imnorm(im):
    ...
    ----------
    im : np.ndarray
        Image to normalize. Must be a single color-channel.
...
def make_cell_fig(recdict, cell_id):
    ...
    bbox_x = ([np.min(xy[0,:]-d),
                np.max(xy[0,:])+d])

    bbox_y = ([np.max(xy[1,:])+d,
                np.min(xy[1,:]-d)])

    # get maximum and mean projections
